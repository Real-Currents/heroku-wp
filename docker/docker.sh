...
#sudo
docker run --name herokuwp-db --rm -p 127.0.0.1:3306:3306 \
...
#sudo
docker run --name herokuwp-php --rm -p 127.0.0.1:9000:9000 \
...
#sudo
docker run --name herokuwp-web --rm -p 127.0.0.1:80:80  \
  --link herokuwp-db:herokuwp --link herokuwp-php:php \
  -v $PWD/public.built:/app/public.built:ro  \
  -v $PWD/support/vagrant/root/etc/ssl:/etc/ssl:ro \
  -v $PWD/support/vagrant/root/etc/nginx/conf.d:/etc/nginx/conf.d:ro \
  -it xqdocker/ubuntu-nginx bash
